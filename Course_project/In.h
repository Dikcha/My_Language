...
namespace In
{
	struct IN
	{
		enum { E = 500, T = 600, F = 700, I = 800, S = 900, D = 1000, K = 1100, C = 1200, B = 1300, A=1400 };//«D» - символы являющиеся сепараторами; //«K» - символы кавычек;//«C» - символы операторов сравнения; //«B» - символы бинарных операторов;//«S» - символ пробела;//«I» - символы, которые игнорируются; //«F» - запрещенные алфавитом символы;	//«T» - разрешенные алфавитом символы(также разрешенными являются все символы кроме «F» и «I»);	//«E» - символ окончания строки;	//T-допустимый символ, F-недопустимый, I-игнорировать, иначе заменить
